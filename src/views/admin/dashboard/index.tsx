        ...
        <Fragment>
            <div className="mb-6 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
                <div className="rounded-lg bg-white p-6 shadow-md">
                    ...
                    <p className="text-3xl font-bold">{statistical_data?.data.total_movies ?? 0}</p>
                </div>
                <div className="rounded-lg bg-white p-6 shadow-md">
                    <h3 className="mb-2 text-lg font-semibold">Tống số tập</h3>
                    <p className="text-3xl font-bold">{statistical_data?.data.total_episodes ?? 0}</p>
                </div>
                <div className="rounded-lg bg-white p-6 shadow-md">
                    <h3 className="mb-2 text-lg font-semibold">Tổng số người dùng</h3>
                    <p className="text-3xl font-bold">{statistical_data?.data.total_users ?? 0}</p>
                </div>
                <div className="rounded-lg bg-white p-6 shadow-md">
                    <h3 className="mb-2 text-lg font-semibold">Phim mới trong tháng</h3>
                    <p className="text-3xl font-bold">{statistical_data?.data.total_movies_create_this_month ?? 0}</p>
                </div>
            </div>

            <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                <div className="p-2 shadow-md">
                    <h3 className="mb-4 text-xl font-semibold">Phim nổi bật theo tuần</h3>
                ...
                <div className="h-max p-2 shadow-md">
                    <h3 className="mb-4 text-xl font-semibold">Phim mới tạo gần đây</h3>
                    <Loading loading={isFetching}>
                        <ul className="scrollbar-none relative max-h-96 min-h-60 overflow-hidden overflow-y-auto rounded">
                            <li className="sticky top-0 flex items-center justify-between bg-black px-2 py-3 text-white">
                                <span>Tên phim</span>
